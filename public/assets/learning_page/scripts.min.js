/******/ (() => {
  ...
  (function ($) {
      ...
      if (quiz.has_expired) {
        ...
        if (quiz.can_try) {
          otherHtml += '\n                <a href="/panel/quizzes/'
            .concat(
              quiz.id,
              '/start" target="_blank" class="btn btn-primary btn-sm mt-15">'
            )
    ...
    function handleLiveSessionHtml(session) {
      ...
      var otherHtml = "";
      if (session.password) {
        otherHtml += '<div class="font-14 font-weight-500 text-gray mt-5">'
          .concat(passwordLang, ": ")
      ...
      if (
        ...
        if (file.online_viewer && file.online_viewer !== "0") {
          ...
          html += '<iframe src="/ViewerJS/index.html#'
            .concat(file.file_path, '" class="file-online-viewer rounded-sm ')
            .concat(
              file.downloadable && file.downloadable !== "0"
                ? "has-download-card"
                : "",
        ...
        switch (file.storage) {
          ...
          case "upload_archive":
            ...
            var _otherHtml = '<a href="'
              .concat(courseUrl, "/file/")
              .concat(
                file.id,
                '/showHtml" target="_blank" class="btn btn-primary btn-sm mt-15">'
              )
              .concat(showLang, "</a>");
    ...
    function handleTextLessonHtml(textLesson) {
      var html =
        '<div class="text-lesson-content p-15 p-lg-30">\n                    <h4 class="font-16 font-weight-bold text-dark">'
          .concat(textLesson.title, "</h4>\n\n                    ")
          .concat(
            textLesson.image
              ? '<div class="pb-5 mt-15 main-image rounded-lg w-100">\n                                <img src="'
                  .concat(textLesson.image, '" class="bg-gray200" alt="')
                  .concat(
                    textLesson.title,
                    '"/>\n                            </div>'
                  )
        ...
        Object.keys(textLesson.attachments).forEach(function (key) {
          ...
          html +=
            '<div class="col-12 col-lg-3 mt-10 mt-lg-0">\n                            <a href="'
              ...
              .concat(attachment.file.file_type, " | ")
              .concat(
        ...
        function (result) {
          if (result && result.code === 200) {
            var storage = result.storage,
              path = result.path;
            var $iframeHtml =
              '<div class="learning-content-iframe">\n                            '.concat(
                path,
                "\n                        </div>"
              );
