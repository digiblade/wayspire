(function ($) {
    ...
    $("#partnerInstructorSwitch").on("change.bootstrapSwitch", function (e) {
        ...
        if (isChecked) {
            ...
            handleSearchableSelect2(
                "js-search-partner-user",
                adminPanelPrefix + "/users/search",
                "name"
            );
        } else {
    ...
    $("body").on("change", "#categories", function (e) {
        ...
        $.get(
            adminPanelPrefix + "/filters/get-by-category-id/" + category_id,
            function (result) {
                if (
                    result &&
                    ...
                    Object.keys(result.filters).forEach((key) => {
                        ...
                        html +=
                            ...
                            '<strong class=" category-filter-title text-light d-block">' +
                            filter.title +
                            "</strong>\n" +
                            '<div class="py-10"></div>\n' +
                            "\n";
    ...
    $(document).ready(function () {
        ...
        function updateToDatabase(table, idString) {
            $.post(
                adminPanelPrefix + "/webinars/order-items",
        ...
        if ($fileForms && $fileForms.length) {
            $fileForms.each((key) => {
                if ($fileForms[key]) {
                    ...
                    const secureHostType = $form
                        .find(".js-secure-host-upload-type-field input:checked")
                        .val();
        ...
        var summernoteTarget = $(
            ".accordion-content-wrapper .js-content-summernote"
        );
            ...
            if (result && result.item) {
                ...
                Object.keys(item).forEach(function (key) {
                    ...
                    if (relation === "textLessons" && key === "content") {
                        var summernoteTarget = $form.find(
                            ".js-content-" + item_id
                        );
    ...
    $("body").on("click", ".js-add-chapter", function (e) {
        ...
        clone.html(
            '<div id="chapterModal' + random + '">' + cloneHtml + "</div>"
        );
        ...
        Swal.fire({
            ...
            onOpen: function () {
                ...
                if (itemId) {
                    ...
                    const path =
                        adminPanelPrefix + "/chapters/" + itemId + "/update";
                    ...
                    $.get(
                        ...
                        function (result) {
                            if (result && result.chapter) {
                                modal
                                    .find(".js-ajax-title")
                                    .val(result.chapter.title);

                                const status = modal.find(
                                    ".js-chapter-status-switch"
                                );
                                ...
                                const checkedAllContents =
                                    result.chapter.check_all_contents_pass &&
                                    result.chapter.check_all_contents_pass !==
                                        "0";
                                modal
                                    .find(".js-chapter-check-all-contents-pass")
                                    .prop("checked", checkedAllContents);

                                var localeSelect =
                                    modal.find(".js-chapter-locale");
                                localeSelect.val(locale);
                                localeSelect.addClass(
                                    "js-webinar-content-locale"
                                );
    ...
    $("body").on(
        "click",
        ".js-add-course-content-btn, .add-new-interactive-file-btn",
        function (e) {
            e.preventDefault();
            const $this = $(this);
            const type = $this.attr("data-type");
                ...
                case "text_lesson":
                    ...
                    if (jQuery().summernote) {
                        makeSummernote(
                            ...
                            400,
                            function (contents, $editable) {
                                $(".js-hidden-content-summernote-" + key).val(
                                    contents
                                );
                            }
                        );
    ...
    function handleShowFileInputsBySource($form, source, fileType) {
        ...
        const $secureHostUploadTypeField = $form.find(
            ".js-secure-host-upload-type-field"
        );
        ...
        switch (source) {
            ...
            case "s3":
                ...
                if (source === "external_link") {
                    icon =
                        feather.icons["external-link"].toSvg(featherIconsConf);
                    $filePathButton.removeClass("panel-file-manager");
        ...
        if (filePathPlaceHolderBySource) {
            $filePathInput.attr(
                "placeholder",
                filePathPlaceHolderBySource[source]
            );
        }
    }
    ...
    $("body").on(
        "change",
        ".js-secure-host-upload-type-field input",
        function (e) {
            e.preventDefault();
    ...
    $("body").on("click", ".assignment-attachments-add-btn", function (e) {
        ...
        copyHtml = copyHtml.replaceAll(
            "assignment-attachments-add-btn",
    ...
    $("body").on("click", "#webinarAddPrerequisites", function (e) {
        ...
        Swal.fire({
            ...
            onOpen: () => {
                handleSearchableSelect2(
                    "prerequisites-select2",
                    adminPanelPrefix + "/webinars/search",
                    "title"
                );
            },
        });
    ...
    $("body").on("click", "#add_new_learning_materials", function (e) {
        ...
        Swal.fire({
            ...
            width: "48rem",
            onOpen: function () {
                $('#extraDescriptionModal input[name="type"]').val(
                    "learning_materials"
                );
            },
        });
        ...
        modalHtml
            ...
            .append(
                '<div class="form-group">\n' +
                    ...
                    '                    <button type="button" class="input-group-text admin-file-manager" data-input="image_' +
                    key +
                    '" data-preview="holder">\n' +
                    '                        <i class="fa fa-upload"></i>\n' +
                    "                    </button>\n" +
                    "                </div>\n" +
                    '                <input type="text" name="value" id="image_' +
                    key +
                    '" class="form-control"/>\n' +
                    "            </div>\n" +
                    "        </div>"
    ...
    $("body").on(
        "change",
        ".js-edit-extraDescription-locale-ajax",
        function (e) {
            e.preventDefault();
            const $this = $(this);
            const locale = $this.val();
        ...
        $.post(
            ...
            function (result) {
                if (result && result.webinarExtraDescription) {
                    const webinarExtraDescription =
                        result.webinarExtraDescription;
                    ...
                    html = html.replaceAll(
                        adminPanelPrefix + "/webinar-extra-description/store",
                        adminPanelPrefix +
                            "/webinar-extra-description/" +
                            item_id +
                            "/update"
                    ...
                    Swal.fire({
                        ...
                        onOpen: () => {
                            ...
                            Object.keys(webinarExtraDescription).forEach(
                                (key) => {
                                    $modal
                                        .find('[name="' + key + '"]')
                                        .val(webinarExtraDescription[key]);
                                }
                            );

                            var localeSelect = $modal.find(
                                'select[name="locale"]'
                            );
    ...
    function editTicket($this, locale = null) {
        ...
        $.post(
            adminPanelPrefix + "/tickets/" + ticket_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.ticket) {
                    ...
                    edit_ticket_modal = edit_ticket_modal.replaceAll(
                        adminPanelPrefix + "/tickets/store",
                        adminPanelPrefix + "/tickets/" + ticket_id + "/update"
                    );
                    ...
                    Swal.fire({
                        ...
                        onOpen: () => {
                            ...
                            var localeSelect = $("#addTicketModal").find(
                                'select[name="locale"]'
                            );

                            if (localeSelect) {
                                localeSelect.addClass(
                                    "js-edit-ticket-locale-ajax"
                                );
    ...
    function editChapter($this, locale = null) {
        ...
        $.post(
            adminPanelPrefix + "/chapters/" + chapter_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.chapter) {
                    ...
                    html = html.replaceAll(
                        adminPanelPrefix + "/chapters/store",
                        adminPanelPrefix + "/chapters/" + chapter_id + "/update"
                    );
                        ...
                        onOpen: () => {
                            ...
                            Object.keys(chapter).forEach((key) => {
                                if (key === "status") {
                                    ...
                                    const checkedAllContents =
                                        chapter.check_all_contents_pass &&
                                        chapter.check_all_contents_pass !== "0";
                                    $modal
                                        .find('[name="' + key + '"]')
                                        .prop("checked", checkedAllContents);
                                } else {
    ...
    function editSession($this, locale = null) {
        ...
        $.post(
            adminPanelPrefix + "/sessions/" + session_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.session) {
                    ...
                    edit_session_modal = edit_session_modal.replaceAll(
                        adminPanelPrefix + "/sessions/store",
                        adminPanelPrefix + "/sessions/" + session_id + "/update"
                    );
                    const nameId = randomString();
                    edit_session_modal = edit_session_modal.replaceAll(
                        "record",
                        ...
                        onOpen: () => {
                            ...
                            Object.keys(session).forEach((key) => {
                                if (key === "session_api") {
                                    ...
                                    key === "check_previous_parts" ||
                                    key === "access_after_day"
                                ) {
                                    const sequenceContentSwitchChecked =
                                        session.check_previous_parts ||
                                        session.access_after_day !== null;

                                    if (sequenceContentSwitchChecked) {
                                        $modal
                                            .find(".js-sequence-content-switch")
                                            .prop("checked", true);
                                        $modal
                                            .find(
                                                '[name="check_previous_parts"]'
                                            )
                                            .prop(
                                                "checked",
                                                session.check_previous_parts
                                        ...
                                        $modal
                                            .find('[name="access_after_day"]')
                                            .val(session.access_after_day);
                                        $modal
                                            .find(".js-sequence-content-inputs")
                                            .removeClass("d-none");
                                    }
                                } else if (key === "agora_settings") {
                                    const agora_settings = JSON.parse(
                                        session.agora_settings
                                    );

                                    if (
                                        agora_settings &&
                                        agora_settings["chat"] &&
                                        (agora_settings["chat"] === true ||
                                            agora_settings["chat"] === "true")
                                    ) {
                                        $modal
                                            .find('[name="agora_chat"]')
                                            .prop("checked", true);
                                    }

                                    if (
                                        agora_settings &&
                                        agora_settings["record"] &&
                                        (agora_settings["record"] === true ||
                                            agora_settings["record"] === "true")
                                    ) {
                                        $modal
                                            .find('[name="agora_record"]')
                                            .prop("checked", true);
                                    }
                                } else {
                                    $modal
    ...
    $("body").on("change", ".js-video-demo-source", function (e) {
        ...
        if (value === "secure_host") {
            ...
            const $filePathUploadButton = $(
                ".js-video-demo-path-input .js-video-demo-path-upload"
            );
            const $filePathLinkButton = $(
                ".js-video-demo-path-input .js-video-demo-path-links"
            );
            ...
            if (videoDemoPathPlaceHolderBySource) {
                $filePathInput.attr(
                    "placeholder",
                    videoDemoPathPlaceHolderBySource[value]
                );
            }
        }
    });
    ...
    $("body").on("click", ".edit-prerequisite", function (e) {
        ...
        $.post(
            adminPanelPrefix + "/prerequisites/" + prerequisite_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.prerequisite) {
                    ...
                    edit_prerequisite_modal =
                        edit_prerequisite_modal.replaceAll(
                            adminPanelPrefix + "/prerequisites/store",
                            adminPanelPrefix +
                                "/prerequisites/" +
                                prerequisite_id +
                                "/update"
    ...
    function editFaq($this, locale = null) {
        ...
        $.post(
            adminPanelPrefix + "/faqs/" + faq_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.faq) {
                    ...
                    edit_faq_modal = edit_faq_modal.replaceAll(
                        adminPanelPrefix + "/faqs/store",
                        adminPanelPrefix + "/faqs/" + faq_id + "/update"
                    );
    ...
    $("body").on("click", ".edit-webinar-quiz", function (e) {
        ...
        $.post(
            adminPanelPrefix + "/webinar-quiz/" + webinar_quiz_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.webinarQuiz) {
                    ...
                    edit_webinar_quiz_modal =
                        edit_webinar_quiz_modal.replaceAll(
                            adminPanelPrefix + "/webinar-quiz/store",
                            adminPanelPrefix +
                                "/webinar-quiz/" +
                                webinar_quiz_id +
                                "/update"
                    ...
                    Swal.fire({
                        ...
                        onOpen: () => {
                            ...
                            $("#addQuizModal")
                                .find('[name="chapter_id"]')
                                .val(webinar_quiz.chapter_id);
                        },
                    });
                }
            }
    ...
    $("body").on("change", ".js-sequence-content-switch", function () {
        ...
        const sequenceContentInputs = parent.find(
            ".js-sequence-content-inputs"
        );
    ...
    $("body").on("click", "#bundleAddNewCourses", function (e) {
        ...
        html += "</div>";
        html = html.replaceAll(
            "bundleWebinars-select",
            "bundleWebinars-select2"
        );
    ...
    $("body").on("click", ".edit-bundle-webinar", function (e) {
        ...
        $.post(
            adminPanelPrefix + "/bundle-webinars/" + item_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.bundleWebinar) {
                    ...
                    $("#bundleWebinarsModal .bundleWebinars-select").html(
                        selectHtml
                    );
                    ...
                    html = html.replaceAll(
                        adminPanelPrefix + "/bundle-webinars/store",
                        adminPanelPrefix +
                            "/bundle-webinars/" +
                            item_id +
                            "/update"
