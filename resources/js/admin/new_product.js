(function ($) {
    ...
    function editFile($this, locale = null) {
        ...
        $.post(
            adminPanelPrefix + "/store/products/files/" + file_id + "/edit",
            edit_data,
            function (result) {
            ...
            onOpen: () => {
                ...
                Object.keys(file).forEach((key) => {
                    if (key === "status") {
                        ...
                        const checked =
                            file.online_viewer && file.online_viewer === 1;
                        $modal
                            .find('input[name="' + key + '"]')
                            .prop("checked", checked);
                    } else {
        ...
        $.get(
            ...
            function (result) {
                if (result) {
                    ...
                    const multiValuesInput = $form.find(
                        ".js-multi-values-input"
                    );
                    const summaryInput = $form.find(".js-summery-input");
                    const allowSelectionInput = $form.find(
                        ".js-allow-selection-input"
                    );
                    ...
                    if (specification.input_type === "multi_value") {
                        ...
                        handleSpecificationMultiValueSelect2(
                            "multi_values-select2"
                        );
                    } else {
    ...
    $("body").on("click", "#productAddSpecification", function (e) {
        ...
        html = html.replaceAll(
            ...
            "search-specification-select2-" + key
        );
    ...
    $("body").on("change", "#categories", function (e) {
        ...
        $.get(
            adminPanelPrefix +
                "/store/products/filters/get-by-category-id/" +
                category_id,
            function (result) {
                if (
                    result &&
