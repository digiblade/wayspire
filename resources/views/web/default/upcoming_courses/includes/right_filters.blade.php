<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container text-light">
            ...
            @foreach (['webinar', 'course', 'text_lesson'] as $typeOption)
                <div class="d-flex align-items-center justify-content-between mt-20">
                    <label class="cursor-pointer"
                        for="filterLanguage{{ $typeOption }}">{{ trans('webinars.' . $typeOption) }}</label>
    ...
    <div class="mt-25 pt-25 border-top border-gray300">
        <h3 class=" category-filter-title text-light font-20 font-weight-bold text-dark-blue">
            {{ trans('site.more_options') }}</h3>

        <div class="pt-10">
            @foreach (['supported_courses', 'quiz_included', 'certificate_included'] as $moreOption)
                <div class="d-flex align-items-center justify-content-between mt-20">
                    <label class="cursor-pointer"
                        for="filterLanguage{{ $moreOption }}">{{ trans('update.show_only_' . $moreOption) }}</label>
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" name="moreOptions[]" id="filterLanguage{{ $moreOption }}"
                            value="{{ $moreOption }}" @if (in_array($moreOption, request()->get('moreOptions', []))) checked="checked" @endif
                            class="custom-control-input">
            ...
            <div class="pt-10">
                @foreach ($categoriesLists as $categoryItem)
                    @if (!empty($categoryItem->subCategories) and count($categoryItem->subCategories))
                        ...
                        <div class="pl-10">
                            @foreach ($categoryItem->subCategories as $subCategory)
                                ...
                                <a href="/upcoming_courses?{{ http_build_query($urlParams) }}"
                    ...
                    @else
                        ...
                        <a href="/upcoming_courses?{{ http_build_query($urlParams) }}"
                            class="d-flex align-items-center font-14 font-weight-bold mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $categoryItem->id) ? 'text-primary' : '' }}">
