    ...
    <div class="col-12 col-md-6 mt-15">
            ...
            <div class="form-group mt-30">
                <div class="d-flex align-items-center justify-content-between">
                    <label class="cursor-pointer input-label"
                        for="certificateSwitch">{{ trans('update.bundle_completion_certificate') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="certificate" class="custom-control-input" id="certificateSwitch"
                            {{ (!empty($bundle) && $bundle->certificate or old('certificate') == 'on') ? 'checked' : '' }}>
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="tags" data-max-tag="5"
                value="{{ !empty($bundle) ? implode(',', $bundleTags) : '' }}" class="form-control inputtags"
        ...
        <div class="form-group mt-15">
            ...
            <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror" name="category_id"
                required>
                <option {{ (!empty($bundle) and !empty($bundle->category_id)) ? '' : 'selected' }} disabled>
                        ...
                        <optgroup label="{{ $category->title }}">
                            @foreach ($category->subCategories as $subCategory)
                                <option value="{{ $subCategory->id }}"
                                    {{ (!empty($bundle) and $bundle->category_id == $subCategory->id or old('category_id') == $subCategory->id) ? 'selected' : '' }}>
                                    {{ $subCategory->title }}</option>
                            @endforeach
                        </optgroup>
                    @else
                        <option value="{{ $category->id }}"
                            {{ (!empty($bundle) and $bundle->category_id == $category->id or old('category_id') == $category->id) ? 'selected' : '' }}>
