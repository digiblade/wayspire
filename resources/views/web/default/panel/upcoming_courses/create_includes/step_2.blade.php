...
<div class="row">
    <div class="col-12 col-md-6 mt-15">
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" max="100" name="course_progress"
        ...
        @if (!empty(getCertificateMainSettings('status')))
            <div class="form-group mt-30 d-flex align-items-center justify-content-between">
                <label class="cursor-pointer input-label"
                    for="certificateSwitch">{{ trans('update.certificate_included') }}</label>
                <div class="custom-control custom-switch">
        ...
        <div class="form-group mt-30 d-flex align-items-center justify-content-between">
            <label class="cursor-pointer input-label"
                for="quizzesSwitch">{{ trans('update.include_quizzes') }}</label>
            <div class="custom-control custom-switch">
        ...
        <div class="form-group mt-15">
            ...
            <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror"
                ...
                <option {{ (!empty($upcomingCourse) and !empty($upcomingCourse->category_id)) ? '' : 'selected' }}
                    disabled>{{ trans('public.choose_category') }}</option>
                @foreach ($categories as $category)
                    @if (!empty($category->subCategories) and $category->subCategories->count() > 0)
                        <optgroup label="{{ $category->title }}">
                            @foreach ($category->subCategories as $subCategory)
                                <option value="{{ $subCategory->id }}"
                                    {{ (!empty($upcomingCourse) and $upcomingCourse->category_id == $subCategory->id or old('category_id') == $subCategory->id) ? 'selected' : '' }}>
                                    {{ $subCategory->title }}</option>
                            @endforeach
                        </optgroup>
                    @else
                        <option value="{{ $category->id }}"
                            {{ (!empty($upcomingCourse) and $upcomingCourse->category_id == $category->id or old('category_id') == $category->id) ? 'selected' : '' }}>
