                ...
                <div class="col-12 col-lg-4">
                    <div class="form-group">
                        ...
                        <select name="blog_id" class="form-control select2"
                            ...
                            <option {{ empty($selectedPost) ? 'selected' : '' }} value="">{{ trans('public.all') }}
                            </option>

                            @foreach ($posts as $post)
                                <option value="{{ $post->id }}"
                                    {{ (!empty($selectedPost) and $selectedPost->id == $post->id) ? 'selected' : '' }}>
                                    {{ $post->title }}</option>
                            @endforeach
                        </select>
                    </div>
                                    ...
                                    @foreach ($comments as $comment)
                                        <tr>
                                            ...
                                            <td class=" text-left align-middle" width="35%">
                                                <a href="{{ $comment->blog->getUrl() }}" target="_blank"
                                                    class="text-dark-blue font-weight-500">{{ $comment->blog->title }}</a>
                                            </td>
                                            ...
                                            <td class="align-middle">
                                                @if ($comment->status == 'active')
                                                    <span
                                                        class="text-primary font-weight-500">{{ trans('public.active') }}</span>
                                                @else
                                                    <span
                                                        class="text-dark-blue font-weight-500">{{ trans('public.pending') }}</span>
                                                @endif
                                            </td>
                                            <td class="align-middle">
