<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container text-light">
    <h3 class=" category-filter-title text-light font-20 font-weight-bold text-dark-blue">{{ trans('update.filters') }}
    </h3>
    ...
    <div class="form-group">
        ...
        <select name="role" id="instructor_type" class="form-control">
            ...
            <option value="{{ \App\Models\Role::$teacher }}"
                {{ request()->get('role') == \App\Models\Role::$teacher ? 'selected' : '' }}>
                {{ trans('public.instructor') }}</option>
            <option value="{{ \App\Models\Role::$organization }}"
                {{ request()->get('role') == \App\Models\Role::$organization ? 'selected' : '' }}>
                {{ trans('home.organization') }}</option>
        </select>
    </div>
        ...
        <div class="d-flex align-items-center wizard-custom-radio mt-5">
            <div class="wizard-custom-radio-item flex-grow-1">
                <input type="radio" name="meeting_type" value="all" id="all" class=""
                    {{ request()->get('meeting_type') == 'all' ? 'checked' : '' }}>
            ...
            <div class="wizard-custom-radio-item flex-grow-1">
                <input type="radio" name="meeting_type" value="in_person" id="in_person" class=""
                    {{ request()->get('meeting_type') == 'in_person' ? 'checked' : '' }}>
            ...
            <div class="wizard-custom-radio-item flex-grow-1">
                <input type="radio" name="meeting_type" value="online" id="online" class=""
                    {{ request()->get('meeting_type') == 'online' ? 'checked' : '' }}>
        ...
        <div class="d-flex align-items-center wizard-custom-radio mt-5">
            <div class="wizard-custom-radio-item flex-grow-1">
                <input type="radio" name="population" value="all" id="population_all" class=""
                    {{ request()->get('population') == 'all' ? 'checked' : '' }}>
                <label class="font-12 cursor-pointer px-15 py-10"
                    for="population_all">{{ trans('public.all') }}</label>
            </div>

            <div class="wizard-custom-radio-item flex-grow-1">
                <input type="radio" name="population" value="single" id="population_single" class=""
                    {{ request()->get('population') == 'single' ? 'checked' : '' }}>
                <label class="font-12 cursor-pointer px-15 py-10"
                    for="population_single">{{ trans('update.single') }}</label>
            </div>

            <div class="wizard-custom-radio-item flex-grow-1">
                <input type="radio" name="population" value="group" id="population_group" class=""
                    {{ request()->get('population') == 'group' ? 'checked' : '' }}>
                <label class="font-12 cursor-pointer px-15 py-10"
                    for="population_group">{{ trans('update.group') }}</label>
            </div>
        </div>
    </div>
