<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container text-light">
    <h3 class=" category-filter-title text-light font-20 font-weight-bold text-dark-blue">{{ trans('update.location') }}
    </h3>
            ...
            @if (!empty($countries))
                @foreach ($countries as $country)
                    <option value="{{ $country->id }}"
                        {{ request()->get('country_id') == $country->id ? 'selected' : '' }}>{{ $country->title }}
                    </option>
                @endforeach
            @endif
        </select>
            ...
            @if (!empty($cities))
                @foreach ($cities as $city)
                    <option value="{{ $city->id }}"
                        {{ request()->get('city_id') == $city->id ? 'selected' : '' }}>{{ $city->title }}</option>
                @endforeach
            @endif
        </select>
    </div>
