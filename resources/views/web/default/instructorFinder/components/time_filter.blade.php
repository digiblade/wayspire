...
<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container text-light">
    <h3 class=" category-filter-title text-light font-20 font-weight-bold text-dark-blue">{{ trans('public.time') }}</h3>

    <div class="mt-35">
        @foreach ($days as $day)
            <div class="custom-control custom-checkbox mb-20 full-checkbox w-100">
                <input type="checkbox" name="day[]" value="{{ $day }}" class="custom-control-input"
                    id="day_{{ $day }}" {{ in_array($day, $requestDays) ? 'checked' : '' }}>
                <label class="custom-control-label font-14 w-100"
                    for="day_{{ $day }}">{{ trans('panel.' . $day) }}</label>
            </div>
        @endforeach
    </div>
