                        ...
                        <form action="/forums/search" method="get">
                            <div class="form-group d-flex align-items-center m-0">
                                <input type="text" name="search" class="form-control border-0"
                                    placeholder="{{ trans('update.search_discussions') }}" />
                                <button type="submit"
                                    class="btn btn-primary rounded-pill">{{ trans('public.search') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-6 col-md-3">
                    <div class="d-flex align-items-center justify-content-center flex-column">
                        <img src="/assets/default/img/forum/1.svg" alt="{{ trans('update.forums') }}"
                            class="forum-stat-icon" />
                ...
                <div class="col-6 col-md-3">
                    <div class="d-flex align-items-center justify-content-center flex-column">
                        <img src="/assets/default/img/forum/2.svg" alt="{{ trans('update.topics') }}"
                            class="forum-stat-icon" />
                ...
                <div class="col-6 col-md-3">
                    <div class="d-flex align-items-center justify-content-center flex-column">
                        <img src="/assets/default/img/forum/3.svg" alt="{{ trans('site.posts') }}"
                            class="forum-stat-icon" />
                ...
                <div class="col-6 col-md-3">
                    <div class="d-flex align-items-center justify-content-center flex-column">
                        <img src="/assets/default/img/forum/4.svg" alt="{{ trans('update.members') }}"
                            class="forum-stat-icon" />
                ...
                <div class="forums-featured-card d-flex align-items-center bg-white p-20 p-md-35 rounded-lg mt-15">
                    ...
                    <div class="ml-15">
                        ...
                        <div class="mt-15 d-flex align-items-end">
                            @if (
                                $featuredTopic->topic->posts_count > 0 or
                                    !empty($featuredTopic->usersAvatars) and count($featuredTopic->usersAvatars))
                                <div class="forums-featured-card-users-avatar d-flex align-items-center mr-10">
                                    @foreach ($featuredTopic->usersAvatars as $userAvatar)
                                        <div class="user-avatar-card rounded-circle">
                                            <img src="{{ $userAvatar->getAvatar(32) }}" class="img-cover rounded-circle"
                                                alt="{{ $userAvatar->full_name }}">
                                        </div>
                                    @endforeach

                                    @if ($featuredTopic->topic->posts_count - count($featuredTopic->usersAvatars) > 0)
                                        <span
                                            class="topics-count d-flex align-items-center justify-content-center font-12 text-gray rounded-circle">+{{ $featuredTopic->topic->posts_count - count($featuredTopic->usersAvatars) }}</span>
                                    @endif
                                </div>
                            @endif

                            <div class="d-flex align-items-center">
                                <div class="text-gray font-12">{{ trans('public.created_by') }} <span
                                        class="font-weight-bold">{{ $featuredTopic->topic->creator->full_name }}</span>
                                </div>

                                <div class="text-gray font-12 ml-5 pl-5 border-left">
                                    {{ trans('update.n_posts', ['count' => $featuredTopic->topic->posts_count]) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section class="container forums-categories-section mt-30">
            ...
            @foreach ($forums as $forum)
                <div class="forums-categories-card mt-30 rounded-lg border bg-white p-15">
                    <h3 class="forums-categories-card__title text-dark font-16 font-weight-bold mb-15">{{ $forum->title }}
                    </h3>
                    ...
                    <div class="col-12 col-md-3 mt-15">
                        <div class="forum-recommended-topics__card position-relative rounded-lg bg-white px-20 py-30">
                            <div class="forum-recommended-topics__icon">
                                <img src="{{ $recommendedTopic->icon }}" alt="{{ $recommendedTopic->title }}"
                                    class="img-cover">
                            </div>
                            ...
                            <div class="forum-recommended-topics__lists mt-5">
                                @foreach ($recommendedTopic->topics as $topic)
                                    <a href="{{ $topic->getPostsUrl() }}"
                                        class="d-flex align-items-center text-gray font-14 font-weight-500 mt-15">
                                        <i data-feather="chevron-right" class="mr-5 text-primary" width="16"
                                            height="16"></i>
        ...
        <div class="row">
            <div class="col-12 col-md-7">
                <div class="px-10 px-md-25 py-25 p-md-50">
                    <h1 class="font-36 font-weight-bold text-secondary" style="color:white">
                        <span class="d-block ">{{ trans('update.have_a_question?') }}</span>
                        <span class="d-block">{{ trans('update.ask_it_in_forum_and_get_answer') }}</span>
                    </h1>

                    <p class="mt-15 text-light font-14">{{ trans('update.have_a_question_hint') }}</p>
