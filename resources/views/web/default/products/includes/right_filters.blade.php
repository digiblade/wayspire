...
<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container text-light">
    <div class="">
        <h3 class=" category-filter-title text-light font-20 font-weight-bold text-dark-blue">{{ trans('public.type') }}
        </h3>
            ...
            @foreach (['virtual', 'physical'] as $typeOption)
                <div class="d-flex align-items-center justify-content-between mt-20">
                    <label class="cursor-pointer"
                        for="filterTypes{{ $typeOption }}">{{ trans('update.product_type_' . $typeOption) }}</label>
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" name="type[]" id="filterTypes{{ $typeOption }}"
                            value="{{ $typeOption }}" @if (in_array($typeOption, request()->get('type', []))) checked="checked" @endif
...
<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container text-light">
    <div class="">
        <h3 class=" category-filter-title text-light font-20 font-weight-bold text-dark-blue">
            {{ trans('update.options') }}</h3>

        <div class="pt-10">

            <div class="d-flex align-items-center justify-content-between mt-20">
                <label class="cursor-pointer"
                    for="filterOptionsOnlyAvailableProducts">{{ trans('update.only_available_products') }}</label>
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="options[]" id="filterOptionsOnlyAvailableProducts"
                        value="only_available" @if (in_array('only_available', request()->get('options', []))) checked="checked" @endif
                        class="custom-control-input">
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                <label class="cursor-pointer"
                    for="filterOptionsWithPoint">{{ trans('update.products_with_points') }}</label>
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="options[]" id="filterOptionsWithPoint" value="with_point"
                        @if (in_array('with_point', request()->get('options', []))) checked="checked" @endif class="custom-control-input">
                ...
                @foreach ($productCategories as $productCategory)
                        ...
                        <div class="pl-10">
                            @foreach ($productCategory->subCategories as $subCategory)
                                <a href="{{ $subCategory->getUrl() }}"
                                    class="d-flex align-items-center font-14 font-weight-normal mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $subCategory->id) ? 'text-primary' : '' }}">
                    ...
                    @else
                        <a href="{{ $productCategory->getUrl() }}"
                            class="d-flex align-items-center font-14 font-weight-bold mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $productCategory->id) ? 'text-primary' : '' }}">
                ...
                @if (!empty($filter->options))
                    <div class="pt-10">
                        @foreach ($filter->options as $option)
                            <div class="d-flex align-items-center justify-content-between mt-20">
                                <label class="cursor-pointer"
                                    for="filterLanguage{{ $option->id }}">{{ $option->title }}</label>
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" name="filter_option[]"
                                        id="filterLanguage{{ $option->id }}" value="{{ $option->id }}"
                                        @if (in_array($option->id, request()->get('filter_option', []))) checked="checked" @endif
                                        class="custom-control-input">
                                    <label class="custom-control-label"
                                        for="filterLanguage{{ $option->id }}"></label>
                                </div>
                            </div>
                        @endforeach
                    </div>
