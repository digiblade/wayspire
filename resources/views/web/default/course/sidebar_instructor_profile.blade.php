<div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
    ...
    @if (!empty($webinarPartnerTeacher))
        <span
            class="user-select-none px-15 py-10 bg-gray200 off-label text-gray text-white font-12 rounded-sm ml-auto">{{ trans('public.invited') }}</span>
    @endif
    ...
    <div class="mt-25 d-flex flex-row align-items-center justify-content-center w-100">
        <a href="{{ $courseTeacher->getProfileUrl() }}" target="_blank"
            class="btn btn-sm btn-primary {{ $hasMeeting ? 'teacher-btn-action' : 'btn-block' }}">{{ trans('public.profile') }}</a>
