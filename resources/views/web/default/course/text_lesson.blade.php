                ...
                <div class="mt-30 row align-items-center">
                        ...
                        @if (auth()->check())
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" name="read" class="custom-control-input" id="readLessonSwitch"
                                        data-course-id="{{ $course->id }}" data-lesson-id="{{ $textLesson->id }}"
                    ...
                    <div class="col-12 col-md-7 text-right">
                        @if (!empty($course->textLessons) and count($course->textLessons))
                            <a href="{{ !empty($previousLesson) ? $course->getUrl() . '/lessons/' . $previousLesson->id . '/read' : '#' }}"
                                class="btn btn-sm {{ !empty($previousLesson) ? 'btn-primary' : 'btn-gray disabled' }}">{{ trans('public.previous_lesson') }}</a>

                            @if (!empty($nextLesson))
                                <a href="{{ !$nextLesson->not_purchased ? $course->getUrl() . '/lessons/' . $nextLesson->id . '/read' : '#' }}"
                                    class="btn btn-sm {{ !$nextLesson->not_purchased ? 'btn-primary' : 'btn-gray disabled' }} {{ $nextLesson->not_purchased ? 'js-not-purchased' : '' }}">{{ trans('public.next_lesson') }}</a>
                            @endif
                        @endif
                    </div>
                </div>
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
                    <div class="teacher-avatar mt-5">
                        <img src="{{ $course->teacher->getAvatar(100) }}" class="img-cover"
                    ...
                    <div class="shadow-sm rounded-lg bg-white px-15 px-md-25 py-20 mt-30">
                        ...
                        <ul class="p-0 m-0 pt-10">
                            @foreach ($textLesson->attachments as $attachment)
                                <li
                                    class="mt-10 p-10 rounded bg-info-light font-14 font-weight-500 text-dark-blue d-flex align-items-center justify-content-between text-ellipsis">
                                    <span class="">{{ $attachment->file->title }}</span>

                                    <a href="{{ $course->getLearningPageUrl() }}?type=file&item={{ $attachment->file->id }}"
                                        target="_blank">
