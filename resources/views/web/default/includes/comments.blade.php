<div class="mt-35">
    <h2 class="section-title after-line text-light">{{ trans('panel.comments') }} <span
            class="ml-5">({{ $comments->count() }})</span></h2>
    ...
    <div class="mt-20">
        <form action="/comments/store" method="post">
            ...
            <button type="submit" class="btn btn-sm btn-primary bg-blue">{{ trans('product.post_comment') }}</button>
        </form>
    </div>
    ...
    @if ($comments)
        @foreach ($comments as $comment)
            <div class="comments-card shadow-lg rounded-sm border px-20 py-15 mt-30"
                data-address="/comments/{{ $comment->id }}/reply" data-csrf="{{ csrf_token() }}"
                data-id="{{ $comment->id }}">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex align-items-center">
                        <span
                            class="font-12 text-gray mr-10">{{ dateTimeFormat($comment->created_at, 'j M Y | H:i') }}</span>
                        ...
                        <div class="btn-group dropdown table-actions">
                            ...
                            <div class="dropdown-menu">
                                <button type="button"
                                    class="btn-transparent webinar-actions d-block text-hover-primary reply-comment">{{ trans('panel.reply') }}</button>
                                <button type="button" data-item-id="{{ $inputValue }}"
                                    data-comment-id="{{ $comment->id }}"
                                    class="btn-transparent webinar-actions d-block mt-10 text-hover-primary report-comment">{{ trans('panel.report') }}</button>

                                @if (auth()->check() and auth()->user()->id == $comment->user_id)
                                    <a href="/comments/{{ $comment->id }}/delete"
                                        class="webinar-actions d-block mt-10 text-hover-primary">{{ trans('public.delete') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="rounded-sm border px-20 py-15 mt-30">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="user-inline-avatar d-flex align-items-center mt-10">
                                    ...
                                    <div class="d-flex flex-column ml-5">
                                        <span
                                            class="font-weight-500 text-secondary">{{ $reply->user->full_name }}</span>
                                        <span class="font-12 text-gray">
                                            @if (
                                ...
                                <div class="d-flex align-items-center">
                                    <span
                                        class="font-12 text-gray mr-10">{{ dateTimeFormat($reply->created_at, 'j M Y | H:i') }}</span>
