                            ...
                            <form action="/{{ $page }}" method="get">
                                <div class="form-group d-flex align-items-center m-0">
                                    <input type="text" name="search" class="form-control border-0"
                                        ...
                                        placeholder="{{ trans('public.search') }} {{ $title }}" />
                                    <button type="submit"
    ...
    <div class="container">
        ...
        <form id="filtersForm" action="/{{ $page }}" method="get">
            ...
            <div class="mt-30 px-20 py-15 rounded-sm shadow-lg border border-gray300">
                <h3 class=" category-filter-title text-light font-20 font-weight-bold">{{ trans('categories.categories') }}
                </h3>
                        ...
                        @else
                            <div class="checkbox-button bordered-200 mr-20">
                                <input type="checkbox" name="categories[]" id="checkbox{{ $category->id }}"
                                    value="{{ $category->id }}" @if (in_array($category->id, request()->get('categories', []))) checked="checked" @endif>
        ...
        <section>
            ...
            <div class="text-center">
                <button type="button" id="loadMoreInstructors"
                    ...
                    class="btn btn-border-white mt-50 {{ $instructors->lastPage() <= $instructors->currentPage() ? ' d-none' : '' }}">{{ trans('site.load_more_instructors') }}</button>
            </div>
        </section>


        @if (
            !empty($bestRateInstructors) and
                !$bestRateInstructors->isEmpty() and
                (empty(request()->get('sort')) or !in_array(request()->get('sort'), ['top_rate', 'top_sale'])))
        ...
        @if (
            !empty($bestSalesInstructors) and
                !$bestSalesInstructors->isEmpty() and
                (empty(request()->get('sort')) or !in_array(request()->get('sort'), ['top_rate', 'top_sale'])))
                    ...
                    <div id="topSaleInstructorsSwiper" class="swiper-container px-12">
                        <div class="swiper-wrapper pb-20">
                            ...
                            @foreach ($bestSalesInstructors as $bestSalesInstructor)
                                <div class="swiper-slide">
                                    @include('web.default.pages.instructor_card', [
                                        'instructor' => $bestSalesInstructor,
                                    ])
                                </div>
                            @endforeach
