    ...
    <div class="col-12 col-md-6">
        ...
        <div class="form-group mt-3">
            ...
            <div class="input-group">
                ...
                <input type="text" name="publish_date"
                    value="{{ (!empty($upcomingCourse) and $upcomingCourse->publish_date) ? dateTimeFormat($upcomingCourse->publish_date, 'Y-m-d H:i', false, false, $upcomingCourse->timezone) : old('publish_date') }}"
                    class="form-control @error('publish_date')  is-invalid @enderror datetimepicker"
        ...
        <div class="form-group mt-15">
            ...
            <input type="number" name="capacity"
                value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->capacity)) ? $upcomingCourse->capacity : old('capacity') }}"
        ...
        <div class="form-group">
            ...
            <input type="text" name="price"
                value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->price)) ? convertPriceToUserCurrency($upcomingCourse->price) : old('price') }}"
        ...
        <div class="form-group">
            ...
            <div class="input-group">
                ...
                <input type="number" name="duration"
                    value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->duration)) ? $upcomingCourse->duration : old('duration') }}"
                    class="form-control @error('duration')  is-invalid @enderror" min="0" />
                @error('duration')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" name="sections"
                value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->sections)) ? $upcomingCourse->sections : old('sections') }}"
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" name="parts"
                value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->parts)) ? $upcomingCourse->parts : old('parts') }}"
                class="form-control @error('parts')  is-invalid @enderror" />
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" name="course_progress"
                value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->course_progress)) ? $upcomingCourse->course_progress : old('course_progress') }}"
                class="form-control @error('course_progress')  is-invalid @enderror"
                placeholder="{{ trans('update.progress_placeholder') }}" />
            ...
            <div class="col-12 col-md-6">
                ...
                <div class="form-group d-flex align-items-center justify-content-between">
                    <label class="cursor-pointer input-label"
                        for="supportSwitch">{{ trans('webinars.support') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="support" class="custom-control-input" id="supportSwitch"
                            {{ (!empty($upcomingCourse) && $upcomingCourse->support or old('support') == 'on') ? 'checked' : '' }}>
                ...
                <div class="form-group mt-30 d-flex align-items-center justify-content-between">
                    <label class="cursor-pointer input-label"
                        for="certificateSwitch">{{ trans('update.include_certificate') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="certificate" class="custom-control-input" id="certificateSwitch"
                            {{ (!empty($upcomingCourse) && $upcomingCourse->certificate or old('certificate') == 'on') ? 'checked' : '' }}>
                ...
                <div class="form-group mt-30 d-flex align-items-center justify-content-between">
                    <label class="cursor-pointer input-label"
                        for="quizzesSwitch">{{ trans('update.certificate_included') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="include_quizzes" class="custom-control-input"
                            id="quizzesSwitch"
                            {{ (!empty($upcomingCourse) && $upcomingCourse->include_quizzes or old('quizzes') == 'on') ? 'checked' : '' }}>
                ...
                <div class="form-group mt-30 d-flex align-items-center justify-content-between">
                    ...
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="downloadable" class="custom-control-input"
                            id="downloadableSwitch"
                            {{ (!empty($upcomingCourse) && $upcomingCourse->downloadable or old('downloadable') == 'on') ? 'checked' : '' }}>
                ...
                <div class="form-group mt-30 d-flex align-items-center justify-content-between ">
                    ...
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="forum" class="custom-control-input" id="forumSwitch"
                            {{ !empty($upcomingCourse) && $upcomingCourse->forum ? 'checked' : (old('forum') ? 'checked' : '') }}>
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="tags" data-max-tag="5"
                value="{{ !empty($upcomingCourse) ? implode(',', $upcomingCourseTags) : '' }}"
        ...
        <div class="form-group mt-15">
            ...
            <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror"
                ...
                <option {{ !empty($upcomingCourse) ? '' : 'selected' }} disabled>{{ trans('public.choose_category') }}
                </option>
                        ...
                        <optgroup label="{{ $category->title }}">
                            @foreach ($category->subCategories as $subCategory)
                                <option value="{{ $subCategory->id }}"
                                    {{ (!empty($upcomingCourse) and $upcomingCourse->category_id == $subCategory->id) ? 'selected' : '' }}>
                                    {{ $subCategory->title }}</option>
                            @endforeach
                        </optgroup>
                    @else
                        <option value="{{ $category->id }}"
                            {{ (!empty($upcomingCourse) and $upcomingCourse->category_id == $category->id) ? 'selected' : '' }}>
...
<div class="form-group mt-15 {{ (!empty($upcomingCourseCategoryFilters) and count($upcomingCourseCategoryFilters)) ? '' : 'd-none' }}"
            ...
            @foreach ($upcomingCourseCategoryFilters as $filter)
                <div class="col-12 col-md-3">
                    <div class="webinar-category-filters">
                        <strong class=" category-filter-title text-light d-block">{{ $filter->title }}</strong>
                        <div class="py-10"></div>

                        @php
                            $upcomingCourseFilterOptions = $upcomingCourse->filterOptions
                                ->pluck('filter_option_id')
                                ->toArray();

                            if (!empty(old('filters'))) {
                                $upcomingCourseFilterOptions = array_merge(
                                    $upcomingCourseFilterOptions,
                                    old('filters'),
                                );
                            }
                        @endphp
                            ...
                            <div class="form-group mt-3 d-flex align-items-center justify-content-between">
                                ...
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" name="filters[]" value="{{ $option->id }}"
                                        {{ !empty($upcomingCourseFilterOptions) && in_array($option->id, $upcomingCourseFilterOptions) ? 'checked' : '' }}
